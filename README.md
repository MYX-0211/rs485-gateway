...
<p align="center"><em>实物接线 —— 面包板上的完整链路：STM32F407 核心板 / MAX485 收发 / SSD1306 OLED / ESP-01S / 两路 RS485 传感器</em></p>
...
```mermaid
flowchart TB
    subgraph BUS["RS485 总线 · USART1 + MAX485 · 9600 8N1"]
        direction LR
        S1["光照传感器<br/>B-RS-L30 · 地址 0x01"]
        S2["温湿度传感器<br/>HKDZ-SHT30 · 地址 0x02"]
    end

    MCU["STM32F407VGT6 @168MHz<br/>FreeRTOS V11.1.0"]

    subgraph TASKS["FreeRTOS 任务（数字为优先级）"]
        direction TB
        T1["vTaskSensor · 3 · 1s<br/>Modbus 轮询 → 报警判定 → 入队"]
        T2["vTaskLED · 2 · 200ms<br/>报警时三灯轮闪"]
        T3["vTaskNet · 1 · 5s<br/>MQTT 上报 / OTA 接收"]
        T4["vTaskOLED · 1 · 200ms<br/>Peek 最新帧刷新显示"]
        T5["vTaskKey · 1 · 10ms<br/>消抖 + 边沿检测"]
    end

    Q[("队列 深 1<br/>xQueueOverwrite")]
    OLED["SSD1306 OLED<br/>I2C1 · 400kHz"]
    ALARM["有源蜂鸣器 + 3×LED"]
    ESP["ESP-01S<br/>USART3 · 115200"]
    W25["W25Q64<br/>SPI1"]
    CLOUD["EMQX Broker<br/>Wi-Fi / TCP 1883"]
    ...
    style CLOUD fill:#d5e8d4,stroke:#82b366
```
...
设备经 ESP-01S 接入 EMQX，用两个主题完成双向通信：

| 主题 | 方向 | QoS | 说明 |
|---|---|---|---|
| `rs485gw/data` | 上行（设备 → 服务端） | 0 | 5 s 周期数据上报，同时承载指令回执 `ack` |
| `rs485gw/cmd` | 下行（服务端 → 设备） | 0 | 阈值设置 / 状态查询 / OTA 触发 |

**上行数据字段**

| 字段 | 类型 | 含义 |
|---|---|---|
| `seq` | uint | 帧序号自增，服务端据此判缺帧 / 重帧（断网补传的前置条件）|
| `up` | uint | 设备自开机毫秒数，用于判断数据新鲜度，与 `seq` 互相印证 |
| `lux` | float | 光照度（Lux）|
| `temp` | float | 温度（℃）|
| `humi` | float | 湿度（%RH）|
| `alarm` | 0 / 1 | 温度越上限报警标志（湿度只采集上报，不参与判定）|

**下行命令**

| 命令 | 回执 | 说明 |
|---|---|---|
| `{"cmd":"set_hi","v":30.0}` | `{"ack":"set_hi","hi":30.0}` | 远程修改报警上限，越界自动钳位；回差同步为 `v - 1.0`，并走与按键同一条掉电保存路径 |
| `{"cmd":"get"}` | `{"ack":"get","hi":…,"lux":…,"temp":…,"humi":…,"alarm":…}` | 查询当前阈值与最新一帧数据，改完可立刻回读，形成闭环验证 |
| `{"cmd":"ota_recv"}` | 串口打印 `STAGED OK` | 断开 MQTT → 连接 PC 的 `TCP:9000` 接收固件 → 校验通过后复位，由 Bootloader 搬运 |
| `{"cmd":"ota"}` / `{"cmd":"ota_bad"}` | `{"ack":"ota","r":0}` | OTA 自测：把当前 APP 写入暂存区；`ota_bad` 故意写错 CRC，用于验证「校验不过时拒绝搬运、旧固件照常运行」 |

> 未识别命令只打印日志并忽略，非法输入不会影响设备运行。`set_hi` 与按键任务可能并发写同一阈值，本工程按「最后写入者胜」处理：两侧都只做「赋值 + 钳位」而不做读-改-写，且 float 单次写入在 Cortex-M4 上是原子的，不会出现撕裂值。

**MQTTX 收发实测**（上行 5 s 一条、`seq` 连续；底部为下行 `ota_recv` 指令）

![MQTTX 收发实测](assets/screenshots/mqttx_report.png)
...
sequenceDiagram
    autonumber
    participant PC as PC端 ota_sender.py
    participant ESP as ESP-01S
    participant APP as APP (vTaskNet)
    participant FL as W25Q64 暂存区
    participant BOOT as Bootloader

    Note over APP: 收到 MQTT 的 ota_recv 指令
    APP->>ESP: AT+MQTTDISCONNECT（与 MQTT 分时复用）
    APP->>FL: 整区擦除（64 KB 块擦，约 1.2 s）
    APP->>ESP: AT+CIPSTART 连接 PC:9000
    PC-->>ESP: TCP 建连
    loop 逐行下发 hex 编码固件
        PC->>ESP: hex 数据行 + CRLF
        ESP->>APP: +IPD 数据帧
        APP->>APP: 跳过前缀 → hex 解码 → 累加 CRC16
        APP->>FL: 写入暂存区
    end
    PC->>APP: 结束帧 END size crc
    APP->>APP: 比对字节数与整包 CRC
    APP->>FL: 回读校验 → 写头部（magic/fw_size/fw_crc/ver）
    Note over APP: 提示后由用户复位

    Note over BOOT: 复位后由 Bootloader 接管
    BOOT->>FL: 读头部
    BOOT->>FL: 校验整包 CRC
    alt CRC 通过
        BOOT->>BOOT: 擦 APP 区 → 按扇区搬运 → 回读校验
        BOOT->>FL: 清除升级标志
        BOOT->>APP: 跳转到 APP 执行
    else CRC 不通过
        BOOT->>APP: 直接跳转（旧固件照常运行）
    end
```
...
```mermaid
flowchart LR
    subgraph MCU["STM32F407 内部 Flash · 1 MB"]
        direction TB
        B["Bootloader<br/>0x08000000<br/>64 KB"]
        A["APP<br/>0x08010000<br/>704 KB"]
        U["未使用"]
    end

    subgraph W25["W25Q64 外部 SPI Flash · 8 MB"]
        direction TB
        S["固件暂存区<br/>0x000000<br/>1 MB"]
        K["备份区（预留）<br/>0x100000<br/>1 MB"]
        R["未使用"]
    end

    PC["PC<br/>ota_sender.py"] -->|"TCP 下发"| S
    S -->|"Bootloader 搬运"| A
    B -->|"校验通过后跳转"| A
    ...
    style K fill:#f5f5f5,stroke:#999999
    style R fill:#f5f5f5,stroke:#999999
    style U fill:#f5f5f5,stroke:#999999
```
...
一次完整的 OTA 升级（48 KB 固件，`ver=2`）：PC 侧 TCP 建连后约 8.8 s 收完，回读 CRC 与接收 CRC 完全一致，暂存落盘成功后才提示复位。
...
[OTA] recv 40960 B
[OTA] recv 49152 B
[OTA] END frame: size=49152 crc=0x6F75
[OTA] readback crc=0x6F75 recv crc=0x6F75
[OTA] recv done: got 49152 B, exp 49152 B; crc got 0x6F75 exp 0x6F75
[OTA] STAGED OK: size=49152 crc=0x6F75 ver=2 -> reboot to apply
[OTA] TCP closed
[OTA] recv result=0, MQTT will reconnect
[ESP] reconnecting...
```

![OTA 升级串口日志](assets/screenshots/ota_uart_log.png)
...
**OLED 显示**（SSD1306，4 行 × 16 字符；正常页 200 ms 刷新，设置页由 KEY1 进入、KEY2/KEY3 调节）

| 正常页 | 阈值设置页 |
|---|---|
| <img src="assets/screenshots/oled_display.jpg" width="300" alt="OLED 正常页显示光照与温湿度"> | <img src="assets/screenshots/oled_threshold_setting.jpg" width="300" alt="OLED 温度阈值设置页"> |
| `S1:30.5 Lux`<br>`S2:24.7C 46.7%`<br>`A:ON  B:ON`<br>`ALARM:OFF` | `==TEMP SET==`<br>`HI:33.0C`<br>`LO:32.0C`<br>`K2:+ K3:-` |

（左：光照/温湿度数值 + 两路在线状态 + 报警状态；右：报警上限 `HI` 与回差解除线 `LO` 相差 1.0℃，调节范围有上下界钳制）
